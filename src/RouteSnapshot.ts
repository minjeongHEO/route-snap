import { RoutePoint, RouteSnapshotConfig, RouteSnapshotOptions } from "./types";
...
export default class RouteSnapshot {
  ...
  private drawRoute() {
    ...
    this.routes.forEach((point, index) => {
      const x =
        canvasPaddingX +
        ((point.longitude - lngMin) / (lngMax - lngMin)) *
          (canvasWidth - 2 * canvasPaddingX);
      const y =
        canvasHeight -
        canvasPaddingY -
        ((point.latitude - latMin) / (latMax - latMin)) *
          (canvasHeight - 2 * canvasPaddingY);
